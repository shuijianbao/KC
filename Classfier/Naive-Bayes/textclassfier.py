...
def Textprocessing(folder_path,test_size = 0.2):
    ...
    train_data_list,train_class_list = zip(*train_list) #测试集解压缩
...
def words_dict(all_words_list,deleteN,stopwords_set = set()):
    ...
    for t in range(deleteN,len(all_words_list),1):
        if n > 1000:
            break
        #如果这个词不是数字，并且不是指定的stopwords,并且单词长度大于1小于5，那么这个词就可以作为特征词
...
def TextClassifier(train_feature_list,test_feature_list,train_class_list,feature_class_list):
...
def TextFeatures1(train_data_list, test_data_list, feature_words):
    ...
    for text in train_data_list:
        features = text_features(text,feature_words)
